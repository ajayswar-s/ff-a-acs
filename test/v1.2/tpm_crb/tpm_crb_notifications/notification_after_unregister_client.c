...
uint32_t notification_after_unregister_client(uint32_t test_run_data)
{
    ...
    LOG(DBG, "Requesting TPM locality %u after unregistering notification\n",
        TPM_TEST_LOCALITY);
    ...
    {
        LOG(ERROR, "Locality %u was not granted after unregister, status=0x%x\n",
    ...
    LOG(DBG, "Relinquishing TPM locality %u after unregister notification check\n",
        TPM_TEST_LOCALITY);
...
cleanup:
    ...
    LOG(TEST, "TPM notification was not delivered after unregister_from_notification\n");
    return VAL_SUCCESS;
}
